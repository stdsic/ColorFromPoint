...
- OS의 프레임버퍼에서 원시 RGB 값을 읽어옵니다.
  - 다시 말해, 위 프로그램에서 조사된 색상값은 장치(모니터)가 사용하는 색 공간에 의존적입니다.
  - 현대의 모니터는 대부분 sRGB 색 공간을 기본으로 하므로 감마 적용된 sRGB 값이 조사됩니다.
  - 브라우저나 포토샵에서 사용하는 문서는 ICC 프로파일이 적용되어 보정된 색상을 사용합니다.
  - 이러한 문서에서 색상값을 조사할 경우 RGB값을 기준으로 +-5 정도의 차이가 발생할 수 있습니다.
  - 일반적으로 브라우저가 화면에 그리는 색상은 GPU 렌더링을 거쳐 감마 및 색 보정이 적용되어 있습니다. 즉, CSS 또는 HTML에서 직접 지정한 문서상의 이론적인 색상과 동일하지 않습니다.
  - 감마 및 색 보정된 값을 그대로 읽어오는 동작을 하여 최종적으로 화면에 출력될 때의 색상값을 조사합니다. 브라우저에 따라 차이가 심할 수 있으며 특히 노란색 계열에서 그 차이가 두드러집니다.
