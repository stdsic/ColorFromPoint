	...
	switch (iMessage) {
		case WM_CREATE:
			try {
				...
				ConvertLength = WideCharToMultiByte(CP_ACP, 
                        0, 
                        wMyMouseProc, 
                        -1, 
                        NULL, 
			...
			for (int i = 0; i < nEdit; i++) {
				hControls[i] = CreateWindowEx(WS_EX_CLIENTEDGE,
                        L"MyEditClass",
                        TEXT(""), 
                        WS_CHILD | WS_VISIBLE | WS_BORDER | ES_AUTOHSCROLL | ES_RIGHT | ES_READONLY,
                        0,0,0,0,
                        hWnd, 
                        (HMENU)(INT_PTR)(IDC_EDSTART + i),
                        GetModuleHandle(NULL),
                        NULL);
			}

			hControls[nControls - 1]= CreateWindowEx(WS_EX_CLIENTEDGE,
                    L"listbox",
                    NULL,
                    WS_CHILD | WS_VISIBLE | WS_BORDER | WS_VSCROLL | LBS_NOTIFY | LBS_OWNERDRAWFIXED,
                    0,0,0,0,
                    hWnd,
                    (HMENU)(INT_PTR)IDC_LBSTART,
                    GetModuleHandle(NULL), 
                    NULL);
		...
		case WM_SIZE:
			if (wParam != SIZE_MINIMIZED) {
				...
				SetWindowPos(hControls[nControls-1],
                        NULL, 
                        x,
                        y,
                        Width, 
				...
				case IDC_LBSTART:
					switch (HIWORD(wParam)) {
						case LBN_SELCHANGE: {
								...
								EllipseColor = (COLORREF)SendMessage(hControls[nControls - 1],
                                        LB_GETITEMDATA, 
                                        idx, 
                                        0);
		...
		case WM_INITMENU:
			if (bLine)
				CheckMenuItem(GetSubMenu((HMENU)wParam, 0),
                        IDM_LINE,
		...
		case WM_TIMER:
			switch (wParam) {
				case 1: {
                        ...
                        HBRUSH hEllipseOldBrush	= (HBRUSH)SelectObject(g_hMemDC, hEllipseBrush);

						Ellipse(g_hMemDC, 
                                EllipseOrigin.x - g_iRadius,
                                EllipseOrigin.y - g_iRadius, 
                                EllipseOrigin.x + g_iRadius,
                                EllipseOrigin.y + g_iRadius);
								...
								switch (VKCode) {
									case 0x33:
										if (GetKeyState(VK_CONTROL) & 0x8000 &&
											...
											StretchBlt(g_hCaptureMemDC,
													0, 
                                                    0, 
                                                    bmp.bmWidth * g_XScale, 
                                                    bmp.bmHeight * g_YScale,
													g_hMemDC,
													0, 
                                                    0,
                                                    (bmp.bmWidth / g_Rate) * g_XScale, 
                                                    (bmp.bmHeight / g_Rate) * g_YScale,
													SRCCOPY);
									...
									case 0x34:
										if (GetKeyState(VK_CONTROL) & 0x8000 &&
                                                GetKeyState(VK_MENU) & 0x8000) {
											if (g_hDrawBitmap) {
												SendMessage(hControls[nControls-1],
                                                        LB_INSERTSTRING,
                                                        0, 
                                                        (LPARAM)SelectColor);
											}
		...
		case WM_DESTROY:
			...
			if (g_hCaptureMemDC)
                DeleteDC(g_hCaptureMemDC);

			if (g_hScreenMemDC)
                DeleteDC(g_hScreenMemDC);

			if (g_hDrawMemDC)
                DeleteDC(g_hDrawMemDC);
